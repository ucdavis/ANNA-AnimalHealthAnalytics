...
wd = 'C:/Apache24_Lepto/anna_lepto/assets/lepto-classifier/' #Working Directory

class LeptoClassifier():
    def __init__(self,):
        with open(wd + 'data/svm_exclude_mat.pkl', 'rb') as f:
            self.svm_exclude_mat = pickle.load(f)
        with open(wd + 'data/svm_include_mat.pkl', 'rb') as f:
            self.svm_include_mat = pickle.load(f)
        self.sd = pd.read_csv(wd + 'data/sd.csv')
        self.prior = pd.read_csv(wd + 'data/prior.csv')
